...
func (h *TicketHandler) GetMany(ctx *fiber.Ctx) error {
	...
	tickets, err := h.repository.GetMany(context) //when authenticating have it as
...
func (h *TicketHandler) GetOne(ctx *fiber.Ctx) error {
	...
	ticket, err := h.repository.GetOne(context, uint(ticketId)) //authing => ticket, err := h.repository.GetOne(context, userId, uint(ticketId))
	...
	QrCode, err = qrcode.Encode(
		fmt.Sprintf("ticketId:%v", ticketId), //authing =>	fmt.Sprint("ticketId:%v", "ownerId:%v", ticketId, userId),
...
func (h *TicketHandler) CreateOne(ctx *fiber.Ctx) error {
	...
	// authing => 	ticket, err := h.repository.CreateOne(context, userId, ticket)
...
func (h *TicketHandler) ValidateOne(ctx *fiber.Ctx) error {
	...
	//remember to get back owner id and have this line as => ticket, err := h.repository.UpdateOne(context, validateBody.OwnerId, validateBody.TicketID, validateData)
